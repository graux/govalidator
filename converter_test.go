...
func TestToInt(t *testing.T) {
	tests := []any{"1000", "-123", "abcdef", "100000000000000000000000000000000000000000000", false}
...
func toString(t *testing.T, test any, expected string) {
	res := ToString(test)
	if res != expected {
...
func TestToJSON(t *testing.T) {
	tests := []any{"test", map[string]string{"a": "b", "b": "c"}, func() error { return fmt.Errorf("Error") }}
	expected := [][]string{
		{"\"test\"", "<nil>"},
		{"{\"a\":\"b\",\"b\":\"c\"}", "<nil>"},
